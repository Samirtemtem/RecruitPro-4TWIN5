...
const JobList = () => {
    ...
    const fetchJobs = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          setError("No data available or incorrect format.");
        }
      } catch (error) {
  ...
  const columns = [
    {
      title: "Job Title",
      dataIndex: "title", // Use 'title' from your schema
      render: (text, record) => (
        <div className="d-flex align-items-center file-name-icon">
          <Link to="#" className="avatar avatar-md bg-light rounded">
            {/* You can add an image here if needed */}
            <ImageWithBasePath
              src={`assets/img/icons/${record.image}`} // Ensure 'image' is part of your data
          ...
          <div className="ms-2">
            ...
            <span className="d-block mt-1">{record.department}</span> {/* Display department */}
          </div>
        </div>
      ),
    ...
    {
      title: "Category",
      dataIndex: "department", // Use 'department' from your schema
      render: (text) => <span>{text}</span>, // Display department name
      sorter: (a, b) => a.department.length - b.department.length,
    },

    {
      title: "Posted Date",
      dataIndex: "publishDate", // Use 'publishDate' from your schema
      render: (text) => <span>{new Date(text).toLocaleDateString()}</span>, // Format as date
      sorter: (a, b) => new Date(a.publishDate) - new Date(b.publishDate),
    },
    {
      title: "Deadline",
      dataIndex: "deadline", // Use 'deadline' from your schema
      render: (text) => <span>{new Date(text).toLocaleDateString()}</span>, // Format as date
      sorter: (a, b) => new Date(a.deadline) - new Date(b.deadline),
    },
    {
      title: "Status",
      dataIndex: "status", // Use 'status' from your schema
      render: (text) => <span>{text}</span>, // Display the job status (OPEN, CLOSED, PENDING)
    ...
    },
    {
      ...
      render: () => (
        <div className="action-icon d-inline-flex">
          <Link to="#" className="me-2" data-bs-toggle="modal" data-bs-target="#edit_post">
...
const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/jobs`, {
            ...formData,
...
<Table dataSource={jobData} columns={columns} Selection={true} />
            </div>
          </div>
        </div>
