...
const EmployeeList = () => {
  ...
  const handleEditSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/user/update/${editingEmployee._id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        setData(prevData => prevData.map(emp => (emp._id === updatedEmployee._id ? updatedEmployee : emp)));
    ...
    { field: 'privilege', headerName: 'Privilege', width: 200 },
    {
      ...
      renderCell: (params: any) => (
        <div className="action-icon d-inline-flex">
          ...
          <Link to="#" onClick={() => handleDeleteClick(params.row._id)}>
