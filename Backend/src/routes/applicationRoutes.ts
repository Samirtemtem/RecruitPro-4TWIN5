...
// Controller to fetch candidates grouped by application status
const fetchCandidatesGroupedByStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    const applications = await Application.find()
      .populate('candidate') // Populate candidate details
      .populate('jobPost') // Populate job posts details
      .exec();
...
// Route to handle GET requests for grouped candidates
router.get('/grouped', fetchCandidatesGroupedByStatus);




// Route to fetch applications by JobPostId
router.get('/byJobId/:jobPostId', async (req: Request, res: Response) : Promise<void> => {
  const { jobPostId } = req.params;

  // Validate the jobPostId
  if (!mongoose.isValidObjectId(jobPostId)) {
       res.status(400).json({ message: 'Invalid JobPostId' });
       return;
  }

  try {
      // Fetch applications associated with the JobPostId
       // Fetch applications associated with the JobPostId
       const applications = await Application.find({ jobPost: jobPostId }); // Adjust if userId is different

      // Check if applications were found
      if (!applications.length) {
           res.status(404).json({ message: 'No applications found for this JobPostId' });
           return;
      }
...
// Route to count applications per job post
router.get('/Count/count/:jobPostId', async (req: Request, res: Response) : Promise<void> => {
  try {
      const { jobPostId } = req.params;

      // Count applications for the specific job post
      const jobPostApplicationsCount = await Application.countDocuments({ jobPost: jobPostId });

      // Count total applications
      const totalApplicationsCount = await Application.countDocuments();

      // Calculate percentage of applications for the job post
      const percentage = totalApplicationsCount > 0 
          ? ((jobPostApplicationsCount / totalApplicationsCount) * 100).toFixed(2)
          : 0; // Avoid division by zero

      // Respond with the counts and percentage
      res.status(200).json({
          jobPostId,
          jobPostApplicationsCount,
