...
const JobListingsTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAppliedJobs = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/app/candidates/${userId}/applications`);
        if (!response.ok) {
  ...
  const deleteApplication = async (applicationId: string) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/app/Deleteapplication/${applicationId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
