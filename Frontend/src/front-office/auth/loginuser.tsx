 ...
 const params = new URLSearchParams(window.location.search);
  ...
  useEffect(() => {
    if (errorlogin === "GoogleLoginCancelled") {
      alert(t("LoginUser.Errors.GoogleLoginCancelled"));
    } else if (errorlogin === "LinkedInLoginCancelled") {
      alert(t("LoginUser.Errors.LinkedInLoginCancelled"));
    }
  }, [errorlogin, t]);


  const { token, setToken, setRole, setUserId, updateProfileData, setUser } = useContext(AuthContext);
  useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.data.user.isVerified) {
        setError(t("LoginUser.Errors.EmailNotVerified"));
        return;
      }
      ...
      switch (errorCode) {
        case 'INVALID_CREDENTIALS':
          setError(t("LoginUser.Errors.InvalidCredentials"));
          break;
        case 'USER_NOT_FOUND':
          setError(t("LoginUser.Errors.UserNotFound"));
          break;
        case 'EMAIL_NOT_VERIFIED':
          setError(t("LoginUser.Errors.EmailNotVerified"));
          break;
        case 'INCORRECT_PASSWORD':
          setError(t("LoginUser.Errors.IncorrectPassword"));
          break;
        case 'USER_PASSWORD_EMAIL':
          setError(t("LoginUser.Errors.UserPasswordEmail"));
          break;
        default:
          setError(t("LoginUser.Errors.UnexpectedError"));
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <AuthModal />
      <DefaulHeader2 />
      <Seo pageTitle={t("LoginUser.SeoTitle")} />
      <MobileMenu />
      <section
        className="job-categories ui-job-categories"
        style={{ backgroundColor: "#FFFFFF", margin: "-40px" }}
      >
              ...
              <div className="col-lg-8 col-md-10 col-sm-12 ps-0">
                <div className="row justify-content-center align-items-center">
                  <div className="col-md-10">
                    <form onSubmit={handleLogin} className="p-4 mt-2">
                      <div className="text-center mb-5">
                        <ImageWithBasePath
                          src="assets/img/Logooo.png"
                          className="img-fluid"
                          id="LogoLogin"
                          alt={t("LoginUser.LogoAlt")}
                        />
                      </div>
                      <div className="text-center mb-4">
                        <p className="text-muted">{t("LoginUser.SignInPrompt")}</p>
                      </div>
                      {error && <div className="alert alert-danger">{error}</div>}
                      <div className="mb-3">
                        <label className="form-label">{t("LoginUser.EmailLabel")}</label>
                        <div className="input-group">
                          <input
                            ...
                            placeholder={t("LoginUser.EmailPlaceholder")}
                            required
                          />
                        </div>
                      </div>
                      <div className="mb-3">
                        <label className="form-label">{t("LoginUser.PasswordLabel")}</label>
                        <div className="pass-group">
                          <input
                            type={passwordVisibility.password ? "text" : "password"}
                            className="pass-input form-control border"
                            value={password}
                      ...
                      <div className="text-center mb-4">
                        <h6 className="fw-normal text-dark mb-0">
                          ...
                          <Link to={all_routes.register} className="hover-a">
                            {" "}
                            {t("LoginUser.CreateAccount")}
                          </Link>
                        </h6>
                      </div>
                      <div className="login-or text-center mb-4">
                        <span className="span-or">{t("LoginUser.Or")}</span>
                      </div>
                        ...
                        <div className="text-center flex-fill">
                          ...
                          >
                            <ImageWithBasePath
                              ...
                              alt={t("LoginUser.FaceRecognitionAlt")}
                            />
                          </Link>
                        </div>
                      </div>
