...
useEffect(() => {
  ...
  const fetchJobPostStatistics = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/jobs/job-posts/statistics`); // Replace with your backend API endpoint
...
const fetchUsers = async () => {
  try {
    const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/usersListLatest`);
...
useEffect(() => {
  const fetchCandidates = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/user/get/Lastcandidates`); // Adjust the API endpoint
      const data = await response.json();
      setCandidates(data);
    } catch (error) {
...
useEffect(() => {
  const fetchEmployeeData = async () => {
      try {
          const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/count-employees-by-department`);
...
const [statsApp, setStatsApp] = useState<ApplicationStats | null>(null);
  ...
  useEffect(() => {
    const fetchApplicationStats = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/app/count-applications`);
        if (!response.ok) {
