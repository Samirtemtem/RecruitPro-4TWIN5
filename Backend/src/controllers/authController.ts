...
import { Department, Role } from "../models/types";
      ...
      if (files.cv?.[0]) {
        ...
        if (!allowedTypes.includes(cvFile.mimetype)) {
          return errorResponse(
            res,
            400,
            "Only PDF and Word documents are allowed for CV",
            "INVALID_CV_TYPE"
          );
        }
    ...
    if (socialLinks) {
      try {
        ...
        if (Array.isArray(socialLinksData) && socialLinksData.length > 0) {
          const validSocialLinks = socialLinksData.filter(
          ...
          if (validSocialLinks.length > 0) {
            ...
            if (savedSocialLinks.length > 0) {
              profile.socialLinks = savedSocialLinks.map(
                (link) => link!._id
    ...
    if (education) {
      ...
      if (Array.isArray(educationData) && educationData.length > 0) {
        ...
        if (validEducation.length > 0) {
          ...
          profile.education = savedEducation.map(
            (edu) => edu._id
          ) as Schema.Types.ObjectId[];
          await profile.save();
        }
    ...
    if (experience) {
      ...
      if (Array.isArray(experienceData) && experienceData.length > 0) {
        ...
        if (validExperience.length > 0) {
          ...
          profile.experience = savedExperience.map(
            (exp) => exp._id
          ) as Schema.Types.ObjectId[];
          await profile.save();
        }
    ...
    if (skills) {
      ...
      if (Array.isArray(skillsData) && skillsData.length > 0) {
        ...
        if (validSkills.length > 0) {
          ...
          profile.skills = savedSkills.map(
            (skill) => skill._id
          ) as Schema.Types.ObjectId[];
          await profile.save();
        }
  ...
  try {
    ...
    if (!user.OneTimePassword) {
      return errorResponse(
        res,
        400,
        "No OTP was generated for this user",
        "INVALID_OTP"
      );
    }
...
export const login = async (
  ...
  try {
    ...
    if (
      (user.provider === "linkedin" || user.provider === "google") &&
      !user.password
    ) {
      const randomPassword = generateRandomPassword();
      user.password = randomPassword;
...
export const update2FASettings = async (
  req: Request,
  res: Response
): Promise<any> => {
  try {
