...
def refresh_recommendations_api():
    ...
    # After successfully refreshing recommendations, also sync job alerts
    try:
        # Define a new data structure for the alert sync
        from flask import Request
        from werkzeug.test import EnvironBuilder
        
        # Directly call the function that syncs alerts with recommendation data
        alerts_created = 0
        job_alerts_collection = db['jobAlerts'] if 'jobAlerts' in db.list_collection_names() else db.create_collection('jobAlerts')
        notifications_collection = db['notifications'] if 'notifications' in db.list_collection_names() else db.create_collection('notifications')
        
        # Get the recommendations we just refreshed
        stored_recommendations = recommendations_collection.find_one({'userId': user_id})
        
        if stored_recommendations and 'recommendations' in stored_recommendations:
            recommendations = stored_recommendations['recommendations']
            
            # Get Express backend URL
            EXPRESS_BACKEND_URL = os.environ.get('EXPRESS_BACKEND_URL', 'http://localhost:3000')
            
            # Process each recommendation to create an alert
            for recommendation in recommendations:
                job_id = recommendation.get('jobId')
                job_title = recommendation.get('jobTitle', 'Unknown Job')
                similarity_score = recommendation.get('similarity', 0)
                ...
                if existing_alert:
                    ...
                    logging.info(f"Updated job alert for user {user_id} and job {job_id}")
                else:
                    # Create new alert
                    try:
                        # Try via Express API
                        alert_data = {
                            'userId': user_id,
                            'jobId': job_id,
                            'criteria': f'Recommended: {job_title}',
                            'notifyVia': ['web'],
                            'relevanceScore': similarity_score
                        }
                        ...
                        if response.status_code == 201:
                            logging.info(f"Created job alert via API for user {user_id} and job {job_id}")
                            alerts_created += 1
                        else:
                        ...
                        notifications_collection.insert_one({
                            'text': f'New job recommendation: {job_title} ({int(similarity_score * 100)}% match)',
                            'type': 'RECOMMENDATION',
                            'link': f'/job-single-v3/{job_id}',
                            'createdAt': datetime.now(),
                            'updatedAt': datetime.now()
                        })
                        
                        logging.info(f"Created job alert directly in MongoDB for user {user_id} and job {job_id}")
                        alerts_created += 1
            
            logging.info(f"Created {alerts_created} job alerts during recommendation refresh")
    except Exception as e:
        logging.error(f"Error syncing alerts after refreshing recommendations: {str(e)}")
    
    return jsonify({
        'message': 'Recommendations refreshed successfully',
        'timestamp': result['timestamp'],
        'count': len(result['jobs']),
        'alertsCreated': alerts_created if 'alerts_created' in locals() else 0
    })
...
@app.route('/api/recommendations/sync-alerts', methods=['POST'])
def sync_alerts_with_recommendations():
    """Sync job alerts with the user's current recommendations"""
    data = request.get_json()
    user_id = data.get('candidateId')
    ...
    try:
        ...
        if not stored_recommendations or 'recommendations' not in stored_recommendations:
            return jsonify({
                'message': 'No recommendations found for this user',
                'alertsCreated': 0
            }), 404
        ...
        # Process each recommendation to ensure an alert exists
        for recommendation in recommendations:
            job_id = recommendation.get('jobId')
            job_title = recommendation.get('jobTitle', 'Unknown Job')
            similarity_score = recommendation.get('similarity', 0)
            ...
            # Check if an alert already exists for this job and user
            existing_alert = job_alerts_collection.find_one({
                'userId': user_id,
                'jobId': job_id
            })
            ...
            if existing_alert:
                ...
                # Create a new alert
                try:
                    # Try to create through the Express API first
                    alert_data = {
                        'userId': user_id,
                        'jobId': job_id,
                        'criteria': f'Recommended: {job_title}',
                    ...
                    if response.status_code == 201:
                        ...
                        raise Exception(f"API failed with status {response.status_code}: {response.text}")
                        
                except Exception as e:
                    # If API fails, create directly in MongoDB
                    job_alerts_collection.insert_one({
                        'userId': user_id,
                        'jobId': job_id,
                        'criteria': f'Recommended: {job_title}',
        ...
        return jsonify({
            'message': f'Successfully synced job alerts with recommendations',
            'alertsCreated': alerts_created,
            'totalRecommendations': len(recommendations)
        })
        
    except Exception as e:
        logging.error(f"Error syncing alerts with recommendations: {str(e)}")
        return jsonify({
            'error': 'Failed to sync alerts with recommendations',
            'message': str(e)
        }), 500
