...
const teamLeadDashboard = () => {
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/skills/top-skills`); // Adjust the URL if needed
...
const [counts, setCounts] = useState({
  ...
  const fetchNeeds = async () => {
    try {
      ...
      const filteredNeeds = user
        ? fetchedData.filter(need => {
            const managerId = need?.department_Manager?.id;
            return String(managerId) === String(user.userId); // ✅ Use userId here
          })
        : [];
    ...
    <>
        ...
        <div className="content">
          ...
          <div className="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
            <div className="my-auto mb-2">
              <h2 className="mb-1">Team Lead Dashboard</h2>
              <nav>
      ...
      <div className="card flex-fill">
        <div className="card-header pb-2 d-flex align-items-center justify-content-between flex-wrap">
          ...
          <Link to="/team-lead-dashboard/needs" className="btn btn-light btn-md mb-2">
            View All
          </Link>
        </div>
                ...
                {needs.length > 0 ? (
                  needs.map((need) => (
                    <tr key={need._id}>
                      <td>
                        <div className="d-flex align-items-center">
                          <Link to="/team-lead-dashboard/needs" className="avatar">
                            <ImageWithBasePath
                  ...
                  <tr>
                    <td colSpan={4} className="text-center">No needs available</td>
                  </tr>
                )}
              </tbody>
            </table>
