...
const JobPosts = () => {
  ...
  const userDepartment = profileData?.department || user?.department;
    ...
    const [formData, setFormData] = useState({
        title: "",
        description: "",
        department: userDepartment || "", // Default to user's department
        status: "OPEN",
...
const [loading, setLoading] = useState(true);
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (userDepartment) {
        const filteredJobs = response.data.filter(job => job.department === userDepartment);
        setJobs(filteredJobs);
        console.log(`Showing ${filteredJobs.length} jobs for department: ${userDepartment}`);
      } else {
        setJobs(response.data);
        console.log("No department filter applied - showing all jobs");
      }
