...
const JobPostRequests: React.FC = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const userRequests = fetchedData.filter(request => 
        request.department_Manager?.id === userId
      );
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    try {
      await axios.put(`http://localhost:5000/request/${id}`, { status });
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/request/create', requestData);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/request/${currentRequestId}`, requestData);
  ...
  const columns = [
    {
      ...
      render: (manager: { firstName?: string; lastName?: string; _id?: string } | null) => {
        // If the manager is the current user, show "You"
        if (manager?._id === userId) {
      ...
      <div className="content">
        ...
        <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
          <div className="my-auto mb-2">
            <h3 className="page-title mb-1">Job Post Requests</h3>
            <nav>
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content" style={{ minWidth: "700px" }}>
            <div className="modal-header">
              <h4 className="modal-title">Add Job Post Request</h4>
              <button
                type="button"
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <input 
                    ...
                    value={user ? `${user.firstName} ${user.lastName}` : "Loading user..."}
                    disabled
                  />
                </div>
                <div className="row">
